  ...
  group('Logarte Integration Tests with Production Backend', () {
    ...
    group('Basic Logging Tests', () {
      test('should log plain messages', () async {
        ...
        logarte.log('Test message 2 with error',
        ...
        expect(
            logarte.logs.value.any((log) => log.contents
                .any((content) => content.contains('Test message 1'))),
            isTrue);
      });
    ...
    group('Network Logging Tests', () {
      test('should log successful network requests', () async {
        ...
        try {
          final response =
              await testDio.get('https://jsonplaceholder.typicode.com/posts/1');
        ...
        await Future.delayed(
            Duration(milliseconds: 500)); // Allow time for logging
    ...
    group('Cloud Sync Tests', () {
      test('should sync logs to cloud successfully', () async {
        ...
        logarte.database(
            target: 'test_table', value: 'test_value', source: 'test');
      ...
      test('should retrieve cloud logs', () async {
        ...
        try {
          await logarte.syncToCloud();
          await Future.delayed(
              Duration(seconds: 2)); // Wait for backend processing
    ...
    group('Alert System Tests', () {
      test('should trigger alerts on repeated failures', () async {
        ...
        // Check if alerts were triggered (might be 0 in test environment)
        expect(alertsFired.length, greaterThanOrEqualTo(0));
        print('Alerts fired during test: ${alertsFired.length}');
    ...
    group('Backend API Health Tests', () {
      ...
      test('should validate API key format', () {
        ...
        expect(apiKey.split('_').length, greaterThanOrEqualTo(3));
    ...
    group('Performance Tests', () {
      test('should handle rapid logging without issues', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          if (i % 10 == 0) {
            logarte.database(
                target: 'rapid_test_$i', value: i, source: 'performance_test');
          }
        }
      ...
      test('should handle concurrent network requests', () async {
        ...
        for (int i = 0; i < 10; i++) {
          futures.add(
            testDio
                ...
                .catchError((e) => Response(requestOptions: RequestOptions(path: ''), statusCode: 0)), // Ignore errors
          );
        }
        ...
        final networkLogs = logarte.logs.value.whereType<NetworkLogarteEntry>();
        expect(networkLogs.length,
            greaterThanOrEqualTo(5)); // At least some should succeed
      });
    });
  });
}
